...
// Operator is responsible for applying the disk device database state to the local node.
// Because the main disk server is provided by the vmm, the wave disk device is mainly responsible for
// ensuring the environment allows bootstrap of the disk server endpoint (e.g. mkdirall() the socket path).
type Operator struct {
	client db.Client
	logger *slog.Logger
	syncer *syncer.Syncer

	// nodeId specifies the id of the node, this is used to determine which disk devices must be applieo.
	nodeId string
...
func New(logger *slog.Logger, client db.Client, opts ...Option) *Operator {
	operator := &Operator{
		client:         client,
		logger:         logger.WithGroup("disk-operator"),
		syncer:         syncer.New(logger.WithGroup("disk-operator"), client),
		nodeId:         "undefined",
		updateCycleTTL: 30,
		syncCycleTTL:   30,
	}
...
// WithNodeId specifies the id of the local node. This id is used to identify which disks
