...
func (a *Adapter) Apply(ctx context.Context, id string, config *disk.DiskConfig, cluster *disk.DiskCluster) error {
  configPath := "/etc/drbd.conf"
  _, err := os.Stat("/etc/drbd.conf")
  ...
  data := bytes.Buffer{}
  data.WriteString("# file was generated by the cthul granit storage engine\n")
  data.WriteString("# it will be overwritten, single source of truth is the cthul database\n")
  data.WriteString(fmt.Sprintf("resource %s {\n", id))
  for node := cluster.Nodes {

    data.WriteString(fmt.Sprintf("\ton %s {\n", node))
    data.WriteString(fmt.Sprintf("\t\t"))
}
  data.WriteString("}\n")
  resourcePath := fmt.Sprintf("/etc/drbd.d/%s.res", id)
  err = os.WriteFile(resourcePath, data.Bytes(), 0600)
  if err!=nil {
    return err
  }
