<script lang="ts">
  import { type Serial } from "$lib/sdk/types/wave/v1/serial/message_pb";
  ...
  async function checkAttached() {
    try {
      ...
      for (const [id, domain] of Object.entries(response.domains)) {
        for (const device of domain.config?.serialDevices ?? []) {
...
<div class="w-full h-[500px] flex flex-row justify-between rounded-xl bg-slate-950/20">
  <div class="w-full flex flex-col items-start p-4">
    <h1 class="text-2xl">Name: <span class="font-bold">{serial.config?.name}</span></h1>
    ...
    <div class="w-full flex flex-row gap-8 justify-start">
      <h2 class="flex flex-col items-start">
        ...
        <span class="text-2xl font-bold opacity-50">{serial.node !== "" ? serial.node : "<none>"}</span>
      </h2>
      <h2 class="flex flex-col items-start">
        <span class="text-sm font-bold">Reqnode</span>
        <span class="text-2xl font-bold opacity-50">{serial.reqnode !== "" ? serial.reqnode : "<none>"}</span>
      </h2>
    </div>
  </div>
