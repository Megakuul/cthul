<script lang="ts">
  ...
  import { type Domain, UpdateRequestSchema, CreateRequestSchema, GetRequestSchema, DomainSchema } from "$lib/types/wave/v1/domain/message_pb";
...
<div class="w-11/12 flex flex-col gap-4 p-2 mt-20">
  <div class="w-full h-[500px] flex flex-row rounded-xl bg-slate-950/20">
    <div class="flex flex-col items-start">
      <h1>{domain.config?.name}</h1>
      <h2>{domain.node}</h2>
      <h2>{domain.reqnode}</h2>
      <p>{page.params.id}</p>
    </div>
  </div>
